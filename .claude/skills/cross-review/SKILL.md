...
自走中（開発者が作業を任せると明示したとき、計画に沿った複数 Issue の連続対応、スケジュール実行）は、ユーザ入力ツールが使える環境でも 3 択で止まらず、指定があれば指定の経路、無ければ推奨の選択肢を実行する。
選んだ経路と理由は最後の報告に書く。
並び順は固定です。
...
### クラウド実行環境 (codex / claude CLI を起動できない環境)
