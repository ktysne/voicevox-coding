 ...
 * key は ENGINE のクエリのフィールド名そのものなので、変更するとクエリに反映されなくなる。
 *
 * omitFromCacheKeyWhenDefault: 既定値のときは WAV キャッシュのキーに含めない印。
 * 後から追加したパラメータに付けておくと、既定のまま使っている利用者の
 * 既存キャッシュがカタログ更新で丸ごと無効化されるのを避けられる
 * （既定値なら合成結果は追加前と同じなので、キーを変える必要がない）。
